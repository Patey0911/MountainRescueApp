...
public partial class RescueRegisterPage : ContentPage
{
    public static string Name, Password, Email, CNP, Nr_Telefon, Identification_Id, Mountain;

    String[] mountains_names = { "Bucegi", "Godeanu", "Fagaras", "Parang", "Retezat" , "Semenic", "Sureanu"};
    public RescueRegisterPage()
    {
        InitializeComponent();
    }
    ...
    private async void Button_Clicked(object sender, EventArgs e)
    {
        ...
        Identification_Id = txtCodID.Text;
        Mountain = txtMountain.Text;

        var rescuer_list = await RescuerRepository.GetAllRescuers();

        var rescuer = new RescuerModel(Name, Password, Email, CNP, Nr_Telefon, Identification_Id, Mountain);
        ...
        if (txtPassword.Text == txtPassword_Confirm.Text)
        {
            ...
            if (await EmailVerification(Email))
            {
                ...
                if (await EmailValidation(Email))
                {
                    ...
                    {
                        if (await Identification_Id_Validiation(Identification_Id))
                        {
                            //Save the new rescuer in Firebase

                            await RescuerRepository.Save(rescuer, rescuer_list.Count + 1);
                            await DisplayAlert("Succes", "The rescuer has been added", "OK");
                            await Navigation.PopAsync();
                        }
                        else
                        {
                            await DisplayAlert("Warning!", "The Identification Code is not correct", "OK");
                        }
                    }
                    else
                    {
    ...
    private async Task<bool> Identification_Id_Validiation(String Cod_id)
    {
        // SM-RO-XXXX-YYYY
        Regex CodeRegex = new Regex(@"^SM-RO-[A-Z0-9]{4}-\d{4}$");
        if (string.IsNullOrWhiteSpace(Cod_id))
            return false;

        return CodeRegex.IsMatch(Cod_id.Trim().ToUpper());
    }
    ...
    private void txtCodID_Completed(object sender, EventArgs e)
    {
        txtName.Focus();
    }
}